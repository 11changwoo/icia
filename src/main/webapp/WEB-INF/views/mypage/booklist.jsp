...
<a href="booklist3"><input type="button" value="영화"></a>
			<div id="content" class="8u skel-cell-important">
				...
				<c:forEach items="${booklist }" var="bcheck">
				<c:choose>
						<c:when test="${bcheck.place == null}">   <p style="padding:100px;">
						<!-- 예약이 없다면  member_user 에서 정보를 가져옴 /해당 테이블에는 place는 항상 없음 -->
							${bcheck.name} 님의 펜션 예약이 없습니다.<br /> <a href="main.wo"><input
					...
					<a href="#" class="image featured"><img
						src="${pageContext.request.contextPath}/img/mypage2.jpg" alt="펜션예약확인"></a>
					<p style="width:500px;">
						나의이름 - ${bcheck.name}<br />
						 지역 - ${bcheck.place}<br />
						 펜션이름 -${bcheck.pname}<br />
						 방이름 - ${bcheck.proom}<br />
						 예약자 이름 - ${bcheck.cname}<br />
						  인원 - ${bcheck.personnel}<br />
						  체크인 - ${bcheck.checkin}<br />
						  체크아웃 - ${bcheck.checkout}<br />						     
						 이메일 - ${bcheck.email}<br />
						  전화번호 - ${bcheck.tel}<br />
						  가격 - ${bcheck.pprice }<br />
					</p>
